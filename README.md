...
- **`prefer-semantic-locator` is a judgement call, not a defect report.** It flags a `locator()`
  ...
  decline. It ships at `info` for that reason, and is 1174 of the 1676 findings these two rules
  produce across the five-repo survey (2744 findings in total). Set it to `off` if your suite has made that call deliberately:
