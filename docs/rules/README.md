...
| [`require-test-tag`](require-test-tag.md) | maintainability | info | A test with no tag cannot be selected by any tag-based run, so it silently misses every suite. |
