...
> snippet` (line/column/severity-independent; whitespace *outside* string literals is normalized so
> re-indentation/line-wraps/formatter spacing never resurface an accepted finding, while text *inside*
> a locator string stays significant so a different selector is still caught), so moving code,
> re-grading a rule, or running a formatter does not create a regression; duplicate findings are
> counted, so an *extra* occurrence beyond the baseline count is treated as new. **`--update-baseline`** (requires `--baseline`) records the current findings to
