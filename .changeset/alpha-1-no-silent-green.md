...
- **Monorepo-friendly discovery — behaviour change:** `testDir` resolves relative to the directory of
  the loaded `testpilot.config.ts` (not the current working directory), for `analyze`, `fix`, and
  `doctor` alike; reported file paths (`findings[].file`, `fix` output, baseline identities) are
...
- `--verbose` prints the resolved discovery base (`[testpilot] files: N under <dir>`).
